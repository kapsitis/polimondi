...
# Bounding rectangle
(xmin, xmax, ymin, ymax) = p.get_bounding_rectangle()
rect_xs = [xmin, xmax, xmax, xmin, xmin]
rect_ys = [ymin, ymin, ymax, ymax, ymin]
rect_ys = [yy * TH for yy in rect_ys]
ax.plot(rect_xs, rect_ys, color='red', linewidth=2)
...
plt.savefig('30gon_bounding_rectangle.svg', format='svg', bbox_inches='tight', transparent=True, pad_inches=0.1)

# Remove hardcoded width and height from the generated SVG
with open('30gon_bounding_rectangle.svg', 'r') as f:
    svg_content = f.read()

import re
svg_content = re.sub(r'width="[0-9.]+pt" height="[0-9.]+pt" ', '', svg_content)

with open('30gon_bounding_rectangle.svg', 'w') as f:
    f.write(svg_content)

print("Saved 30gon_bounding_rectangle.svg")
