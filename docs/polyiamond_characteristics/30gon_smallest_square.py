...
# Call get_smallest_square function
sq_vertices = p.get_smallest_square()

# Draw smallest square
sq_xs = [v[0] for v in sq_vertices] + [sq_vertices[0][0]]
sq_ys = [v[1] for v in sq_vertices] + [sq_vertices[0][1]]
ax.plot(sq_xs, sq_ys, color='red', linewidth=2)

plt.savefig('30gon_smallest_square.svg', format='svg', bbox_inches='tight', transparent=True, pad_inches=0.1)

# Remove hardcoded width and height from the generated SVG
with open('30gon_smallest_square.svg', 'r') as f:
    svg_content = f.read()

import re
svg_content = re.sub(r'width="[0-9.]+pt" height="[0-9.]+pt" ', '', svg_content)

with open('30gon_smallest_square.svg', 'w') as f:
    f.write(svg_content)

print("Saved 30gon_smallest_square.svg")
