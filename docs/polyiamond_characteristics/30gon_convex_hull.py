...
hull = p.convex_hull()
hull_pts = [v.get_xy() for v in hull]
hull_xs = [pt[0] for pt in hull_pts] + [hull_pts[0][0]]
hull_ys = [pt[1] for pt in hull_pts] + [hull_pts[0][1]]

# Draw convex hull with transparent fill
ax.fill(hull_xs, hull_ys, color='green', alpha=0.1)
ax.plot(hull_xs, hull_ys, color='green', linewidth=1.5)

# Draw a little green circle at each convex hull vertex
for pt in hull_pts:
    ax.plot(pt[0], pt[1], 'go', markersize=6)

plt.savefig('30gon_convex_hull.svg', format='svg', bbox_inches='tight', transparent=True, pad_inches=0.1)

# Remove hardcoded width and height from the generated SVG
with open('30gon_convex_hull.svg', 'r') as f:
    svg_content = f.read()

import re
svg_content = re.sub(r'width="[0-9.]+pt" height="[0-9.]+pt" ', '', svg_content)

with open('30gon_convex_hull.svg', 'w') as f:
    f.write(svg_content)

print("Saved 30gon_convex_hull.svg")
