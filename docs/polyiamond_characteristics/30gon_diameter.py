...
# Mark vertices
ax.plot(poly_xs[i_max], poly_ys[i_max], 'ro', markersize=8)
ax.plot(poly_xs[j_max], poly_ys[j_max], 'ro', markersize=8)

# Connect vertices
ax.plot([poly_xs[i_max], poly_xs[j_max]], [poly_ys[i_max], poly_ys[j_max]], color='red', linewidth=2)

plt.savefig('30gon_diameter.svg', format='svg', bbox_inches='tight', transparent=True, pad_inches=0.1)

# Remove hardcoded width and height from the generated SVG
with open('30gon_diameter.svg', 'r') as f:
    svg_content = f.read()

import re
# Removing width="..." and height="..." to allow scalable viewbox
svg_content = re.sub(r'width="[0-9.]+pt" height="[0-9.]+pt" ', '', svg_content)

with open('30gon_diameter.svg', 'w') as f:
    f.write(svg_content)

print("Saved 30gon_diameter.svg")
