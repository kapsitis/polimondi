...
# Call get_closest_hausdorff_triangle function
tri_vertices, dist = p.get_closest_hausdorff_triangle()

# Draw Hausdorff triangle
tri_xs = [v[0] for v in tri_vertices] + [tri_vertices[0][0]]
tri_ys = [v[1] for v in tri_vertices] + [tri_vertices[0][1]]
ax.plot(tri_xs, tri_ys, color='red', linewidth=2)

plt.savefig('30gon_hausdorff_triangle.svg', format='svg', bbox_inches='tight', transparent=True, pad_inches=0.1)

# Remove hardcoded width and height from the generated SVG
with open('30gon_hausdorff_triangle.svg', 'r') as f:
    svg_content = f.read()

import re
svg_content = re.sub(r'width="[0-9.]+pt" height="[0-9.]+pt" ', '', svg_content)

with open('30gon_hausdorff_triangle.svg', 'w') as f:
    f.write(svg_content)

print("Saved 30gon_hausdorff_triangle.svg")
